...
class Sms
{
    ...
    public function __construct()
    {
        $this->client = new Client([
            "base_uri"  => "https://api.marketplace.buupass.com",
            "timeout"   => 10
        ]);
    }
    ...
    {
        //return '';
        $sms = $this->client->post("/sms", ["form_params" => [
            "message" => $message,
    ...
    public function send_instructions($booking, Payment $payment, array $data, string $mode)
    {
        $mode = ucwords($mode);
        $booking = new $mode($booking);
